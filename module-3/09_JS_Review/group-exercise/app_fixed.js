...
document.addEventListener('DOMContentLoaded', () => { // FIX: DOMContentLoaded instead of loaded
    console.log('Application Starting...');

    thingsMattThinksAreFun = thingsMattThinksAreFun.map(thing => {
        let newThing = {...thing, module: getModuleNumFromWeekNum(thing.week) }; // FIX: weekNum was undefined
        console.debug('translating thing', thing, newThing);
        return newThing; // FIX: Was not returning anything from the arrow function, so it mapped it to undefined
    });

    console.log(thingsMattThinksAreFun);

    let app = document.getElementById('app'); // FIX: getElementById does not use CSS selector syntax
    thingsMattThinksAreFun.forEach(thing => {
        ...
        app.appendChild(para); // FIX: Append to app, not para
    });
});