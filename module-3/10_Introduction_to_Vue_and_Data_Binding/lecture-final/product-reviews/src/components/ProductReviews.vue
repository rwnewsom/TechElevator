<template>
  <main>
    ...
    <p>
        We have {{ reviews.length }} reviews with an average rating of 
        {{ averageRating }}
    </p>
...
export default { // I'm basically a class representing the component
    name: 'ProductReviews',
    data() { // These are our fields
        // This returns the starting data for the component (like a constructor)
        return {
    ...
    computed: {
        ...
        averageRating() {
            ...
            //  - a piece of data
            //  - a computed value (derviced property)
            ...
            return total / this.numberOfReviews;
        },
        filteredReviews() {
            let result = this.reviews.slice(); // Copy the array

            // First, if the user has typed into searchText, do a search
            if (this.searchText) {
                result = result.filter(rev => rev.text.toLowerCase().includes(this.searchText.toLowerCase()));
            }

            // If the user has set minimumRating, filter ratings < that #
