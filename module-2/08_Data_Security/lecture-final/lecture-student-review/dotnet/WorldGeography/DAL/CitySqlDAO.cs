        ...
        public IList<City> GetAllCities()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand command = new SqlCommand("SELECT id, name, countrycode, district, population FROM city ORDER BY name, countrycode, district", conn);
            ...
            {
                Console.WriteLine("Couldn't get cities. Nobody is home: " + ex.Message);
            }
