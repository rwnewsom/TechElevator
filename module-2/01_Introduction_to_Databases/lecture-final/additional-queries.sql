-- The name and population of all cities in the USA with a population of greater than 1 million people (9 rows)
SELECT
	name,
	population
FROM
...
WHERE
	population > 1000000
	-- AND countrycode = 'CHN'
	AND countrycode IN (SELECT code FROM country WHERE name = 'China')
...
FROM
	country
WHERE
	gnp > 1000000 -- 1 gnp is $1 million
...
FROM
	country
WHERE
	continent IN ('Africa', 'Asia')
	AND lifeexpectancy >= 70
	AND gnp BETWEEN 1000 AND 100000
	--AND gnp >= 1000 -- gnp is in $1,000 units
	--AND gnp <= 100000