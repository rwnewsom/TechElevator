...
2. For each exercise, navigate to the corresponding project, and provide the code necessary to implement the features specified above in the `Main` method.
3. To verify that your application works as expected, select the project name from the dropdown in the menu bar that is displayed next to the execute button, and then click the button to run the program. Alternatively, you should be able to right-click the project name, then select `Debug -> Start new instance`, which displays the console for your application.
